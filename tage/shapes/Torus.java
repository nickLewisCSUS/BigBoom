...
public class Torus extends ObjShape
{
	...
	{	int numVertices = (prec+1)*(prec+1);
		...
		{	float amt = (float) toRadians(i*360.0f/prec);
			 ...
			 *
			 * <p>**Old Method:** First created a vertext at (0, outer, 0), then rotated it
			 * around the Z-axis and added the inner radius offset.</p>
			 *
			 ...
			 *
			 * <p>**New Method:** Instead of rotating, I calculate the position directly using trig.</p>
			 *
			 * <p>**X-Axis:** `inner + outer * cos(amt)`</p>
			 * <ul>
			 * 	<li> `inner` places the tube around the main torus ring.</li>
			 * 	<li> `outer * cos(amt)` shifts the vertex left or right within the tube circle.</li>
			 * </ul>
			 *
			 * <p>**Y-Axis:** `outer * sin(amt)`</p>
			 * <ul>
			 * 	<li> Moves the vetex up or down within the tube circle.</li>
			 * </ul>
			 *
			 * <p>**Z-Axis:** `0.0f`</p>
			 * <ul>
			 * 	<li> At this stage, the torus is flat in the XY plane, no depth adjustment yet.</li>
			 * </ul>
			 *
			 * @author Tyler Burguillos
			 */
			Vector3f initPos = new Vector3f(inner + outer * (float) cos(amt), outer * (float) sin(amt), 0.0f);
			
			vertices[i].set(initPos);
			
			float u = (1.0f + (float) cos(amt)) * 0.05f;
			float v = (float) i / (float) prec;
			//texCoords[i].set(u,v);
			texCoords[i].set(0.0f, ((float)i)/((float)prec));

			tTangents[i] = new Vector3f(0.0f, -1.0f, 0.0f);
			//tTangents[i].rotateAxis(amt+(float) Math.PI / 2.0f, 0.0f, 0.0f, 1.0f);
		...
		{	for (int i=0; i<prec+1; i++)
			{	float amt = (float) toRadians(ring * 360.0f / prec);
				...
				/**
				 * Adjusts the texture mapping to prevent stretching.
				 * 
				 * <p>**Issue:** The original line used a scaling factor of `2.0f` for the U-coordinate,
				 * which caused the texture to repeat too quickly and appear stretched. </p>
				 *
				 * <p>**Original code:** texCoords[ring*(prec+1)+i].set((float)ring*2.0f/(float)prec, texCoords[i].y()); </p>
				 *
				 * <p>**Fix:** Removed the `2.0f` multiplier so the texture evenly maps around the torus.</p>
				 * @author Tyler Burguillos
				 */
				texCoords[ring*(prec+1)+i].set((float)ring/(float)prec, texCoords[i].y());
