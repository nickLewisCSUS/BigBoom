...
public class GameObject
{
	...
	/** 
	 * moves the object along it's local forward-facing vector 
	 *
	 * @author Tyler Burguillos
	 */
	public void move(float speed) {        
        Vector3f loc = this.getWorldLocation();
        Vector3f fwd = this.getWorldForwardVector();
        Vector3f newLocation = loc.add(fwd.mul(speed));
        this.setLocalLocation(newLocation);
	}
	
	/** 
	 * moves the object along it's local right-facing vector 
	 *
	 * @author Tyler Burguillos
	 */
	public void strafe(float speed) {        
        Vector3f loc = this.getWorldLocation();
        Vector3f right = this.getWorldRightVector();
        Vector3f newLocation = loc.add(right.mul(speed));
        this.setLocalLocation(newLocation);
	}
	
	/** 
	 * rotates the object about the global Y-axis (world up vector). 
	 *
	 * @author Tyler Burguillos
	 */
	public void yaw(float angle) {
		Matrix4f rotation = new Matrix4f().rotate(angle, 0, 1, 0);
		this.setLocalRotation(rotation.mul(this.getLocalRotation()));
	}
	
	/** 
	 * rotates the object about it's local x-axis
	 *
	 * @author Tyler Burguillos
	 */
	public void pitch(float angle) {
		Vector3f right = this.getWorldRightVector();
		Matrix4f rotation = new Matrix4f().rotate(angle, right.x(), right.y(), right.z());
		this.setLocalRotation(rotation.mul(this.getLocalRotation()));
	}
	
	/** 
	 * rotates the object about it's local z-axis 
	 *
	 * @author Tyler Burguillos
	 */
	public void roll(float angle) {
		Vector3f fwd = this.getWorldForwardVector();
		Matrix4f rotation = new Matrix4f().rotate(angle, fwd.x(), fwd.y(), fwd.z());
		this.setLocalRotation(rotation.mul(this.getLocalRotation()));
	}
