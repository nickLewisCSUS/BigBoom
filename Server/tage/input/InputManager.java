...
//		return controllerActions.get(c).put(component, action);
	...
	/**
	 * Clears all input bindings from the input manager/
	 * <p>
	 * This method removes all keyboard and gamepad input bindings,
	 * effectively disabling all player controls. It can be useful for
	 * pausing the game, transitioning between states, or preventing
	 * unwanted input processing during certain events (e.g., game over).
	 * <p>
	 * If needed, individual input actions (such as exiting the game) should
	 * be re-associated after calling this method.
	 * <p>
	 * Example usage:
	 * <pre>
	 ...
	 */
	 public void clearAllInputs() {
		 //Clear all component bindings for each controller
		 for (Controller c : controllers) {
			 controllerActions.get(c).clear();
			 controllerStates.get(c).clear();
		 }
	 }
}
